-- ============================================
-- 004 - INTERACTIONS, ALERTES & ESTIMATIONS
-- Tables: favoris, alertes, messages,
--         conversations, estimations, contacts_agents
-- ============================================
...
CREATE TABLE IF NOT EXISTS alertes (
    ...
    -- Filtres de l'alerte
    type_transaction ENUM('vente', 'location', 'viager') NULL,    -- ✅ ENUM
    type_bien ENUM(                                                -- ✅ ENUM
        'appartement', 'maison', 'villa',
        'studio', 'loft', 'bureau',
        'commerce', 'terrain', 'garage'
    ) NULL,
    ville_id BIGINT UNSIGNED NULL,                                 -- ✅ FK vers villes
    ville_texte VARCHAR(150) NULL,                                 -- fallback si pas de FK
    ...
    nb_envois INT UNSIGNED NOT NULL DEFAULT 0,                     -- ✅ ajouté compteur

    created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP        -- ✅ ajouté
        ON UPDATE CURRENT_TIMESTAMP,

    INDEX idx_alertes_user_actif (user_id, actif),
    INDEX idx_alertes_frequence_actif (frequence, actif),          -- ✅ pour le cron job

    CONSTRAINT fk_alertes_user
        FOREIGN KEY (user_id) REFERENCES users(id)
        ON DELETE CASCADE,
    CONSTRAINT fk_alertes_ville
        FOREIGN KEY (ville_id) REFERENCES villes(id)
        ON DELETE SET NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

-- -----------------------------------------------
-- MESSAGES (contact générique)
-- -----------------------------------------------
CREATE TABLE IF NOT EXISTS messages (
    id BIGINT UNSIGNED AUTO_INCREMENT PRIMARY KEY,

    -- Expéditeur (connecté ou anonyme)
    expediteur_id BIGINT UNSIGNED NULL,                            -- NULL si anonyme
    ...
    agent_id BIGINT UNSIGNED NULL,                                 -- assigné à
    ...
    INDEX idx_messages_expediteur_id (expediteur_id),              -- ✅ ajouté

    CONSTRAINT fk_messages_expediteur
        FOREIGN KEY (expediteur_id) REFERENCES users(id)
        ON DELETE SET NULL,
    CONSTRAINT fk_messages_bien
        FOREIGN KEY (bien_id) REFERENCES biens(id)
        ON DELETE SET NULL,
...
-- -----------------------------------------------
-- CONVERSATIONS (thread d'un message)
-- -----------------------------------------------
CREATE TABLE IF NOT EXISTS conversations (
    ...
    contenu TEXT NOT NULL,
    is_internal TINYINT(1) NOT NULL DEFAULT 0,                     -- note interne agent
    created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,

    INDEX idx_conversations_message_created (message_id, created_at),

    CONSTRAINT fk_conversations_message
        FOREIGN KEY (message_id) REFERENCES messages(id)
        ON DELETE CASCADE,
    CONSTRAINT fk_conversations_auteur
...
CREATE TABLE IF NOT EXISTS estimations (
    ...
    bien_id BIGINT UNSIGNED NULL,                                  -- si bien existant
    ...
    transaction ENUM('vente', 'location', 'viager') NULL,          -- ✅ ENUM
    ...
    -- Structure attendue :
    -- ["parking", "cave", "terrasse", "piscine", "gardien"]
    ...
    rapport_url VARCHAR(2048) NULL,                                -- ✅ élargi
    ...
    INDEX idx_estimations_user_id (user_id),                       -- ✅ ajouté

    CONSTRAINT fk_estimations_user
        FOREIGN KEY (user_id) REFERENCES users(id)
        ON DELETE SET NULL,
    CONSTRAINT fk_estimations_bien
        FOREIGN KEY (bien_id) REFERENCES biens(id)
        ON DELETE SET NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

-- -----------------------------------------------
-- CONTACTS AGENTS (contact ciblé vers un agent)
-- -----------------------------------------------
CREATE TABLE IF NOT EXISTS contacts_agents (
    ...
    traite_at TIMESTAMP NULL,                                      -- ✅ horodatage traitement

    created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,

    INDEX idx_contacts_agents_agent_traite (agent_id, traite),
    INDEX idx_contacts_agents_bien_id (bien_id),                   -- ✅ ajouté

    CONSTRAINT fk_contacts_agents_agent
        FOREIGN KEY (agent_id) REFERENCES users(id)
        ON DELETE CASCADE,
    CONSTRAINT fk_contacts_agents_bien
        FOREIGN KEY (bien_id) REFERENCES biens(id)
        ON DELETE SET NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
